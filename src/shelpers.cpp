...
std::vector<Command> getCommands(const std::vector<std::string>& tokens) {
  std::vector<Command> ret(std::count(tokens.begin(), tokens.end(), "|") +
                           1);  // 1 + num |'s commands
  ...
  for (int i = 0; i < ret.size(); ++i) {
    ...
    for (int j = first + 1; j < last; ++j) {
      if (tokens[j] == ">" || tokens[j] == "<") {
        ...
        if (tokens[j] == ">") {
          ...
          ret[i].fdStdout =
              open(tokens[j + 1].c_str(), O_CREAT | O_WRONLY | O_TRUNC, 0666);
          if (ret[i].fdStdout < 0)
    ...
    if (first < tokens.size()) {
      last =
          std::find(tokens.begin() + first, tokens.end(), "|") - tokens.begin();
    }
  }
